...
这个工作区用于让 LLM 直接合成·维护 raw 源，并将其培育成**永久 Markdown 维基**——不是每次都重新检索的 RAG，而是合成一次后持续累积、保持最新。
...
3. 执行 `/compile` → 读取 inbox 源，在 `30-wiki/` 中合成为源摘要·实体·概念，更新路由器(index)·类型索引·aliases·overview，再把原文移动到 `20-raw/`。
4. 用 `/query` 提问(两级路由·引用)，用 `/lint` 巡检矛盾·孤儿·索引一致性。

**高级 — 复杂文档质量:** 表格·版式较多的 PDF/PPT 可用 LlamaParse(https://cloud.llamaindex.ai)提升转换质量 — 有免费额度，超出后收费。设置好 `LLAMA_CLOUD_API_KEY` 后，`/ingest` 会在复杂文档上自动启用；没有密钥则回退到本地 markitdown。

**核心**: `10-inbox/` 里只要还留着文件 = 就是尚未编译的源。

现在要做的: 引导用户把第一份源放进 `10-inbox/`；如果用户直接给出源，则一起放入并运行 `/compile`。
EOF
else
