...
这是一个把 raw 源由 LLM 直接合成·维护，培育成**永久 markdown 维基**的工作区。在 Claude Code 之上实现了 [Karpathy 的 "LLM Wiki" 模式](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)。

与每次提问都重新检索原文的 RAG 不同，它**合成一次并以最新状态累积**。人负责采集·探索·提问，LLM 负责维基的所有写入·整理·交叉引用。

> 不依赖外部技能·插件。只要有这个文件夹，就能在任何 Claude Code 环境下运行。**这是一个拿来用自己的数据填充、边用边强化的起步模板** —— 不是标准答案，而是起点。
...
claude
```
> 首次运行时会自动弹出使用说明 (SessionStart 钩子)。
...
- **规范化** — 把写法不一致(Parasite/寄生虫)归并为 1 个规范名，规范名首字母即分片键。
